...
public class PublicCoursesServiceImpl implements PublicCoursesService {
	...
	public Integer insCourseIdBatch() {
		return publicDao.insCourseIdBatch(courseDao.selPublicCourses());
	}

	@Override
	public List<PublicCourses> selPublicCoursesPage(int cid,Integer pageNum, Integer pageSize) {
		PageHelper.startPage(pageNum, pageSize);
		List<PublicCourses> data = publicDao.selPublicCourseInfoLimit(cid);
		return data;
	}

	@Override
	public List<PublicCourses> selPublicCoursesById(String ids) {
		List<PublicCourses> data = new ArrayList<PublicCourses>();
		...
		if (ids.contains(",")) {
			...
			PublicCourses courses=publicDao.selPublicCoursesInfoById(ids);
			
			if(courses.getPlace()!=null&&!courses.getPlace().equals("")) {
				return publicDao.selPublicCoursesByPlaceAndTime(courses);
			}
			idList.add(Integer.valueOf(ids));
		}
		return publicDao.selPublicCoursesById(idList);
	}

	@Override
	public Integer selPublicCourseCount() {
		return publicDao.selPublicCourseCount();
	}
	...
	public List<Hfuu_Teacher> selTeacherByTime(String tid, String place, String sid, String week, String whichDay,
		...
		for (int i = 0; i < data.size(); i++) {
			...
			//监考时间
			List<String> coursetime=publicDao.selTeacherTimeByTid(tid);
		...
		if (!freelist.isEmpty() && freelist.contains(list.get(0)) && freelist.contains(list.get(1))) {
			...
			while(teacher.size()<1) {
				...
				for (Hfuu_Course course:datas) {
					...
					List<String> coursetime=publicDao.selTeacherTimeByTid(teacher2.getId());
					if(!coursetime.isEmpty()) {
						for(String str:coursetime) {
		...
		while (teacher.size()<count) {
			...
			for (int i = 0; i < apTeacher.size(); i++) {
				...
				for (Hfuu_Course course:datas) {
					...
					List<String> coursetime=publicDao.selTeacherTimeByTid(apTeacher.get(i).getId());
					if(!coursetime.isEmpty()) {
						for(String str:coursetime) {
							listTime = ExcelResolverUtils.getRemainTime(listTime,str, whichDay, week);
						}
	...
	@Override
	public Integer selCountPublicCourses() {
		return publicDao.selPublicCourseCount();
	}

	@Override
	public Integer selCountOkPublicCourse() {
		
		return publicDao.selCountOkPublicCourse();
	}
	...
	public boolean selTeacherCidByCid(String id, Integer ucid) {
		return publicDao.selTeacherCidByCourseId(id)==ucid?true:false;			
	}

	@Override
	public Integer updPublicCourseInfo(String id, String number, String place, String time, String pid, String fapid,
			String sapid) {
		return publicDao.updPublicCourseById(id, number, place, time, pid, fapid, sapid);
	}

	@Override
	public Integer updPublicCoursesReset(String id) {
	     List<String> list=Arrays.asList(id.split(","));
	     List<String> ids=new ArrayList<String>(list);
	     for(int i=0;i<ids.size();i++) {
	    	 PublicCourses courses=publicDao.selPublicCoursesInfoById(ids.get(i));
	    	
	    	 if(courses.getNumber()==null||courses.getNumber().equals("")) {
	    		 continue;
	    	 }
	    	 List<PublicCourses> data=publicDao.selPublicCoursesByPlaceAndTime(courses);
	    	 for(PublicCourses course:data) {
	    		 publicDao.updPublicCourseResetById(course.getId());
	...
	@Override
	public List<String> selPublicCoursesWeek() {
		return publicDao.selPublicCourseWeek();
	}

	@Override
	public List<String> selCourseName(String week) {
		if(week.equals("0")) {
			return publicDao.selPublicCourseName();
		}
		return publicDao.selPublicCourseNameByWeek(week);
	}

	@Override
	public List<ExportExcel> selPublicCoursesByWeekAndSname(String week, String courseName) throws ParseException {
		List<PublicCourses> srcData=publicDao.selPublicCourseByWeekAndSname(week, courseName);
		List<ExportExcel> distData=new ArrayList<ExportExcel>();
		System.out.println(srcData);
		for(int i=0;i<srcData.size();i++) {
			PublicCourses pCourses=srcData.get(i);
			ExportExcel excel=new ExportExcel();
			for(int j=i+1;j<srcData.size();j++) {
				PublicCourses sCourses=srcData.get(j);
			...
			excel.setTime(distDate+"  第"+pCourses.getTime().split(":")[0]+"周周"+pCourses.getTime().split(":")[1]+" "+pCourses.getTime().split(":")[2]+"节");
	...
	@Override
	public List<ExportExcel> selCoursesInfo(String id) throws ParseException {
		List<PublicCourses> srcData=publicDao.selPublicCourseInfoByTeacher(id);
		srcData.addAll(professionalDao.selProfessionalCourseInfoByTeacher(id));
		System.out.println(srcData);
		List<ExportExcel> distData=new ArrayList<ExportExcel>();
		if(!srcData.isEmpty()) {
		for(int i=0;i<srcData.size();i++) {
